...
class WeatherScheduler:
    ...
    def update_schedule(self, push_time: str):
        ...
        try:
            ...
            logger.warning(f"[Scheduler] Cron: 每天 {hour:02d}:{minute:02d}, 预计下次: {next_run}")
            ...
            logger.warning(f"[Scheduler] ✅ 定时推送已设置: 每天 {push_time}")
        except Exception as e:
            logger.error(f"[Scheduler] ❌ 设置定时任务失败: {e}", exc_info=True)

    async def _execute_callback(self):
        """执行回调函数（由调度器调用）"""
        logger.warning(f"[Scheduler] 🚀 定时推送触发，当前时间: {datetime.now(self.timezone)}")
        if self._callback:
            try:
                await self._callback()
                logger.warning("[Scheduler] ✅ 定时推送执行成功")
            except Exception as e:
    ...
    def start(self):
        ...
        if not self.scheduler.running:
            ...
            logger.warning("[Scheduler] 调度器已启动")
        else:
    ...
    def shutdown(self):
        ...
        if self.scheduler.running:
            ...
            logger.warning("[Scheduler] 调度器已关闭")
